...
if(!$wsp_auth->check_access_token_m(safe_get("access_token"))){
  $status_array = array("message"=>"Sorry. Validation Error","code"=>"401","status"=>"failure");
  // $status_array_response["Response"] = $status_array;
  // $response["response"] = $status_array;           
  $final_json = json_encode($status_array);
  print_r($final_json);
  exit();
}else{
  //Get User Details
...
$where_array = array("booking_user_id"=>$user_id,"booking_status"=>array("Waiting","seated"),"date(date_created)"=>get_today_date()); // You can change it as per required
$options_array = array("orderBy"=>"record_id DESC","limit"=>1,"offset"=>$offset);
//End Configurations
//Search By Columns
...
if(isset($_GET["search"])){
  ...
  $search_operators = array();  //Example $search_operators = array("booking_name"=>"LIKE","date_created"=>"BETWEEN");
  $nested_where = array();
  foreach($search_array as $single_search_column=>$single_search_value){
    ...
    if(array_key_exists($single_search_column,$search_operators)){
      ...
      else if($search_operators[$single_search_column] == "BETWEEN"){
...
//Pass thru custom functions
  ...
  // }
  function date_created($single_column,$dynamic_value,$record_id,$data_array){    
    return beautify_date($dynamic_value,"beauty");
  }
...
function pass_thru($single_column,$dynamic_value,$record_id,$data_array){
  ...
  $allowed_columns = array("date_created");
