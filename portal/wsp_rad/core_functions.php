...
function timeRanges($start_time,$end_time,$interval_in_minutes){
    ...
    while($begin < $end) {
        ...
        $begin->modify('+'.$interval_in_minutes.' minutes');          /** Note, it modifies time by 15 minutes */
        // $output .= $begin->format('H:i');
...
function add_time_to_time($input_time,$type,$how_many,$output = "24"){
    //type can be days, months, years. 
    //input date must be yyyy-mm-dd
    $time = strtotime($input_time);
$change_time = date("H:i a", strtotime('+'.$how_many." ".$type, $time));
...
function add_time_to_time_without_am_pm($input_time,$type,$how_many,$output = "24"){
    //type can be days, months, years. 
    //input date must be yyyy-mm-dd
    $time = strtotime($input_time);
$change_time = date("H:i", strtotime('+'.$how_many." ".$type, $time));
...
function subtract_time_from_time($input_time,$type,$how_many,$output = "24"){
    //type can be days, months, years. 
    //input date must be yyyy-mm-dd
    $time = strtotime($input_time);
$change_time = date("H:i a", strtotime('-'.$how_many." ".$type, $time));
...
function subtract_time_from_time_without_am_pm($input_time,$type,$how_many,$output = "24"){
    //type can be days, months, years. 
    //input date must be yyyy-mm-dd
    $time = strtotime($input_time);
$change_time = date("H:i", strtotime('-'.$how_many." ".$type, $time));
...
function get_rec_meta_with_where($record_type,$where_arr,$options_array){
    global $records_fetch_controller;
    
    $where_array = $where_arr;
    $fetch_rec_meta = $records_fetch_controller->fetch_record_with_where([$record_type,$where_array,$options_array]);
