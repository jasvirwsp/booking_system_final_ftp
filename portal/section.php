                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    ...
                                    <h4 class="page-title"><?php echo beautify_meta_name(title_case("section")); ?> <!-- Button trigger modal -->
<button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#add_section"><i class="fe-plus"></i> Add <?php echo beautify_meta_name(title_case("section"));?></button>
</h4>
                                </div>
                            </div>
                        </div>     
...
$columns_to_exclude = array("record_id");
//Action Buttons
$edit_button_url = "edit_section.php?section_id=";
$view_button_url = "view_section.php?section_id=";
$delete_button_class = "delete_section";
//Records
$per_page = 20;
//Offset settings
...
<table class="table table-striped <?php if (safe_get('show_all_fields') == 'yes') {echo 'table-responsive';} ?>" id="list_table_section">
<?php
...
<!-- Col 1 -->
        ...
        <a href="javascript:void" class="btn btn-secondary btn-sm export_to_csv_section"><i class="fa fa-download" aria-hidden="true"></i> CSV</a>
         <a href="javascript:void" class="btn btn-secondary btn-sm export_to_xls_section"><i class="fa fa-download" aria-hidden="true"></i> Excel</a>
         <a href="javascript:void" class="btn btn-secondary btn-sm print_table_section"><i class="fa fa-print" aria-hidden="true"></i> Print</a>
         <?php 
         if($enable_show_all_fields){
         if (!isset($_GET["show_all_fields"])) { ?>
                    <a href="section.php?show_all_fields=yes" class="btn btn-secondary btn-sm"><i class="fa fa-print" aria-hidden="true"></i> Show All Fields</a>
                  <?php } }?>
        </div>
<!-- Col 1 ends -->
</div>
...
<!-- Add section Start-->
<div class="modal fade" id="add_section" tabindex="-1" role="dialog" aria-labelledby="add_section_LongTitle" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Add <?php echo beautify_meta_name(title_case("section"));?></h5>
      ...
      <!-- name>i --&gt<!-- Form Row Starts -->
        <form method="post" action="" id="form_section_rad64660d8b849db" class="w-100">
        <div class="row">
 <div class="form-group ic_section_name col-md-6 d-inline-block">
         <label class="label_section_name" for="section_name">Name</label>
        <input type="text" class="form-control section_name" placeholder="Name"  name="section_name">
            </div>
        
        <div class="form-group col-md-12">
        <input type="submit" class="btn btn-success btn-sm insert_section" value="Add Section" form_id="rad64660d8b849db"> 
        </div>
        </form>
        </div>
        <!-- Form Row Ends -->
...
<!-- Quick Edit Modal for section Start-->
<div class="modal fade" id="quick_edit_modal_section" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Update <?php echo beautify_meta_name(title_case("section")); ?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">×</span>
        </button>
      </div>
      <div class="modal-body ic_age_modal">
      <form method="post" action="" id="form_update_section_quick_edit" class="w-100">
      <input type="hidden" name="record_id" value="" class="quick_edit_section_record_id">
      <div class="modal_input_loader_section">

      </div>
      <div class="form-group col-md-12">
       
        <input type="submit" class="btn btn-success btn-sm update_section" value="Update <?php echo beautify_meta_name(title_case("section")); ?>" form_id="quick_edit"> 
        </div>
        </form>
      </div>
      <div class="modal-footer">
...
<!-- Quick Edit Modal section Ends-->
    ...
    //Search Filters Conditional changes
        ...
        //Insert variables
        var insert_button_class_section = ".insert_section"; // .classname
                var form_id_section = "#form_section";  // #entry_form
                var insert_api_url_section = "api/section/insert_section.php"; // /api/insert.php
                var form_id_parameter_name = "form_id";
                
                //Insert Operation
                $(document).on("click",insert_button_class_section,function(e){                
                e.preventDefault();
                //Processing
                swal("Processing","Please Wait","warning");
                //Processing
                var form_to_process_section = form_id_section + "_" + $(this).attr(form_id_parameter_name);
        
                $.post(insert_api_url_section,$(form_to_process_section).serializeArray(),function(data){
              ...
              icon: "error"
                    ...
                    $(form_to_process_section + " input[type=text]").first().focus();
...
customized_content_section.innerHTML = "<a href='edit_section.php?section_id="+entry_id_section+"' class='btn btn-sm btn-warning mt-1'>Edit Section</a> <a href='#' onclick='location.reload()' class='btn btn-sm btn-warning mt-1'>Add New Section</a> <!--<a href='#' class='btn btn-sm btn-success mt-1'>Another Action</a> <a href='#' class='btn btn-sm btn-info mt-1'>Another Action</a> --><br><br><a href='#' onclick='var loc = location.href.replace(location.hash,\"\");window.location.assign(loc);' class='btn btn-sm btn-danger'>Close & Refresh</a> <a href='#' onclick='swal.close()' class='btn btn-sm btn-danger'>X</a>";
swal({
    title: "Success",
...
var records_table_id = "#list_table_section";
$(".export_to_csv_section").click(function(){
$(records_table_id).tableToCSV();
});

$(".export_to_xls_section").click(function(){
...
$(".print_table_section").click(function(){
...
var delete_record_button_class_section = ".delete_section"; // .classname
var primary_record_key_attribute_name_section = "record_id"; //Name of attribute of particular record to edit/update/view/delete.
var delete_api_url_section = "api/section/delete_section.php"; // /api/delete.php

$(document).on("click",delete_record_button_class_section,function(e){
e.preventDefault();
...
.then((willDelete) => {
if (willDelete) {
  var record_id_section = $(this).attr(primary_record_key_attribute_name_section);
    
  $.post(delete_api_url_section,{"record_id":record_id_section});
...
.then((willDelete) => {
  ...
  var api_url = "api/section/delete_section.php"; //Replace with correct API url
...
//Quick Edit Code for section starts
$(".quick_edit_section").click(function(){
          var quick_edit_section_record_type = 'section';
          var quick_edit_section_record_id = $(this).attr("record_id");
          $(".quick_edit_section_record_id").val(quick_edit_section_record_id);
          var quick_edit_section_input_name = $(this).attr("quick_edit_input_name");
          $("#quick_edit_modal_section").modal("show");
          $(".modal_input_loader_section").load("edit_"+quick_edit_section_record_type+".php?"+quick_edit_section_record_type+"_id="+quick_edit_section_record_id+" .ic_"+quick_edit_section_input_name);
        });
        

//Update Variables for section
var qe_update_button_class_section= ".update_section"; // .classname
var qe_form_id_section = "#form_update_section";
var qe_update_api_url_section = "api/section/update_section.php"; // /api/update.php
 var qe_form_id_parameter_name_section = "form_id";
    //Update Operation
    	   $(document).on("click",qe_update_button_class_section,function(e){
e.preventDefault();
    		  var qe_form_to_process_section = qe_form_id_section + "_" + $(this).attr(qe_form_id_parameter_name_section);

    	 $.post(qe_update_api_url_section,$(qe_form_to_process_section).serializeArray(),function(data){
        var qe_response_json_section = data;

if(qe_response_json_section["status"] == "failure"){
  var qe_get_errors_section = qe_response_json_section["errors"];
 
    var errors_returned_section = "";
  $(qe_get_errors_section).each(function(index,value){
            errors_returned_section = errors_returned_section + " => " + value;
    });
    ...
    swal("Error",errors_returned_section, {
      icon: "error"
        ...
        //Reset Form
    		//$(qe_form_to_process_section)[0].reset();

        

    		 // Other functions to execute on success
    		   //Customized buttons
var qe_customized_content_section = document.createElement("div");
qe_customized_content_section.innerHTML = "<a href='javascript:void(0)' onClick='location.reload()' class='btn btn-sm btn-warning mt-1'>Refresh Changes</a> <br><br><a href='javascript:void(0)' onclick='swal.close()' class='btn btn-sm btn-danger'>Close Box</a>";
swal({
    title: "Updated",
    text: "Choose next action",
  content: qe_customized_content_section,
  buttons: false,
  icon : 'success',
});
//Customized buttons
...
//Quick Edit Code for section ends 
        </script>
