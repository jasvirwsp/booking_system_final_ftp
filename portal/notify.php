                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    ...
                                    <h4 class="page-title"><?php echo beautify_meta_name(title_case("notify")); ?> <!-- Button trigger modal -->
<button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#add_notify"><i class="fe-plus"></i> Add <?php echo beautify_meta_name(title_case("notify"));?></button>
</h4>
                                </div>
                            </div>
                        </div>     
...
//Action Buttons
$edit_button_url = "edit_notify.php?notify_id=";
$view_button_url = "view_notify.php?notify_id=";
$delete_button_class = "delete_notify";
//Records
$per_page = 20;
//Offset settings
...
<table class="table table-striped <?php if (safe_get('show_all_fields') == 'yes') {echo 'table-responsive';} ?>" id="list_table_notify">
<?php
...
<!-- Col 1 -->
        ...
        <a href="javascript:void" class="btn btn-secondary btn-sm export_to_csv_notify"><i class="fa fa-download" aria-hidden="true"></i> CSV</a>
         <a href="javascript:void" class="btn btn-secondary btn-sm export_to_xls_notify"><i class="fa fa-download" aria-hidden="true"></i> Excel</a>
         <a href="javascript:void" class="btn btn-secondary btn-sm print_table_notify"><i class="fa fa-print" aria-hidden="true"></i> Print</a>
         <?php 
         if($enable_show_all_fields){
         if (!isset($_GET["show_all_fields"])) { ?>
                    <a href="notify.php?show_all_fields=yes" class="btn btn-secondary btn-sm"><i class="fa fa-print" aria-hidden="true"></i> Show All Fields</a>
                  <?php } }?>
        </div>
<!-- Col 1 ends -->
</div>
...
<!-- Add notify Start-->
<div class="modal fade" id="add_notify" tabindex="-1" role="dialog" aria-labelledby="add_notify_LongTitle" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Add <?php echo beautify_meta_name(title_case("notify"));?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">×</span>
        </button>
      </div>
      <div class="modal-body">
      <!-- user_id>i,party_size>i,reminder_status>i --&gt<!-- Form Row Starts -->
        <form method="post" action="" id="form_notify_rad6388836685cd4" class="w-100">
        <div class="row">
 <div class="form-group ic_notify_user_id col-md-6 d-inline-block">
        ...
        <input type="text" class="form-control notify_user_id" placeholder="User Id"  name="notify_user_id">
            </div>
 <div class="form-group ic_notify_party_size col-md-6 d-inline-block">
        ...
        <input type="text" class="form-control notify_party_size" placeholder="Party Size"  name="notify_party_size">
            </div>
 <div class="form-group ic_notify_reminder_status col-md-6 d-inline-block">
        ...
        <input type="text" class="form-control notify_reminder_status" placeholder="Reminder Status"  name="notify_reminder_status">
            </div>
        
        <div class="form-group col-md-12">
        <input type="submit" class="btn btn-success btn-sm insert_notify" value="Add Notify" form_id="rad6388836685cd4"> 
        </div>
        </form>
        </div>
        <!-- Form Row Ends -->
...
<!-- Quick Edit Modal for notify Start-->
<div class="modal fade" id="quick_edit_modal_notify" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Update <?php echo beautify_meta_name(title_case("notify")); ?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">×</span>
        </button>
      </div>
      <div class="modal-body ic_age_modal">
      <form method="post" action="" id="form_update_notify_quick_edit" class="w-100">
      <input type="hidden" name="record_id" value="" class="quick_edit_notify_record_id">
      <div class="modal_input_loader_notify">

      </div>
      <div class="form-group col-md-12">
       
        <input type="submit" class="btn btn-success btn-sm update_notify" value="Update <?php echo beautify_meta_name(title_case("notify")); ?>" form_id="quick_edit"> 
        </div>
        </form>
      </div>
      <div class="modal-footer">
...
<!-- Quick Edit Modal notify Ends-->
    ...
    //Search Filters Conditional changes
        ...
        //Insert variables
        var insert_button_class_notify = ".insert_notify"; // .classname
                var form_id_notify = "#form_notify";  // #entry_form
                var insert_api_url_notify = "api/notify/insert_notify.php"; // /api/insert.php
                var form_id_parameter_name = "form_id";
                
                //Insert Operation
                $(document).on("click",insert_button_class_notify,function(e){                
                e.preventDefault();
                //Processing
                swal("Processing","Please Wait","warning");
                //Processing
                var form_to_process_notify = form_id_notify + "_" + $(this).attr(form_id_parameter_name);
        
                $.post(insert_api_url_notify,$(form_to_process_notify).serializeArray(),function(data){
              ...
              icon: "error"
                    ...
                    $(form_to_process_notify + " input[type=text]").first().focus();
...
var records_table_id = "#list_table_notify";
$(".export_to_csv_notify").click(function(){
$(records_table_id).tableToCSV();
});

$(".export_to_xls_notify").click(function(){
...
$(".print_table_notify").click(function(){
...
var delete_record_button_class_notify = ".delete_notify"; // .classname
var primary_record_key_attribute_name_notify = "record_id"; //Name of attribute of particular record to edit/update/view/delete.
var delete_api_url_notify = "api/notify/delete_notify.php"; // /api/delete.php

$(document).on("click",delete_record_button_class_notify,function(e){
e.preventDefault();
...
.then((willDelete) => {
if (willDelete) {
  var record_id_notify = $(this).attr(primary_record_key_attribute_name_notify);
    
  $.post(delete_api_url_notify,{"record_id":record_id_notify});
...
.then((willDelete) => {
  ...
  var api_url = "api/notify/delete_notify.php"; //Replace with correct API url
...
//Quick Edit Code for notify starts
$(".quick_edit_notify").click(function(){
          var quick_edit_notify_record_type = 'notify';
          var quick_edit_notify_record_id = $(this).attr("record_id");
          $(".quick_edit_notify_record_id").val(quick_edit_notify_record_id);
          var quick_edit_notify_input_name = $(this).attr("quick_edit_input_name");
          $("#quick_edit_modal_notify").modal("show");
          $(".modal_input_loader_notify").load("edit_"+quick_edit_notify_record_type+".php?"+quick_edit_notify_record_type+"_id="+quick_edit_notify_record_id+" .ic_"+quick_edit_notify_input_name);
        });
        

//Update Variables for notify
var qe_update_button_class_notify= ".update_notify"; // .classname
var qe_form_id_notify = "#form_update_notify";
var qe_update_api_url_notify = "api/notify/update_notify.php"; // /api/update.php
 var qe_form_id_parameter_name_notify = "form_id";
    //Update Operation
    	   $(document).on("click",qe_update_button_class_notify,function(e){
e.preventDefault();
    		  var qe_form_to_process_notify = qe_form_id_notify + "_" + $(this).attr(qe_form_id_parameter_name_notify);

    	 $.post(qe_update_api_url_notify,$(qe_form_to_process_notify).serializeArray(),function(data){
        var qe_response_json_notify = data;

if(qe_response_json_notify["status"] == "failure"){
  var qe_get_errors_notify = qe_response_json_notify["errors"];
 
    var errors_returned_notify = "";
  $(qe_get_errors_notify).each(function(index,value){
            errors_returned_notify = errors_returned_notify + " => " + value;
    });
    ...
    swal("Error",errors_returned_notify, {
      icon: "error"
        ...
        //Reset Form
    		//$(qe_form_to_process_notify)[0].reset();

        

    		 // Other functions to execute on success
    		   //Customized buttons
var qe_customized_content_notify = document.createElement("div");
qe_customized_content_notify.innerHTML = "<a href='javascript:void(0)' onClick='location.reload()' class='btn btn-sm btn-warning mt-1'>Refresh Changes</a> <br><br><a href='javascript:void(0)' onclick='swal.close()' class='btn btn-sm btn-danger'>Close Box</a>";
swal({
    title: "Updated",
    text: "Choose next action",
  content: qe_customized_content_notify,
  buttons: false,
  icon : 'success',
});
//Customized buttons
...
//Quick Edit Code for notify ends 
        </script>
